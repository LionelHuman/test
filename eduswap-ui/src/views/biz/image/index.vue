...
<script>
import { listImage, getImage, delImage, addImage, updateImage } from "@/api/biz/image"
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除商品图片编号为"' + imageIds + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('biz/image/export', {
        ...this.queryParams
