<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['biz:collection:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['biz:collection:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['biz:collection:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="collectionList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="收藏ID" align="center" prop="collectId" />
...
<script>
import { listCollection, getCollection, delCollection, addCollection, updateCollection } from "@/api/biz/collection"
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getCollection(collectId).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除收藏编号为"' + collectIds + '"的数据项？').then(function() {
        return delCollection(collectIds)
      }).then(() => {
        this.getList()
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('biz/collection/export', {
        ...this.queryParams
